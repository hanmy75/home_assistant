...
# update apt-get db for node.js
sudo curl -sL https://deb.nodesource.com/setup_22.x | sudo -E bash -

# Install package
sudo apt-get install -y python3 python-dev-is-python3 python3-venv python3-pip libffi-dev libssl-dev libjpeg-dev zlib1g-dev autoconf build-essential libopenjp2-7 libtiff5 tzdata
sudo apt-get install -y git certbot nginx libnginx-mod-rtmp samba samba-common-bin transmission-daemon ffmpeg exfat-fuse nodejs rclone
